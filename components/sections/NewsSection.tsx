      ...
      <Container>
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-16">
          <motion.div variants={organicReveal} initial="hidden" whileInView="visible" className="flex items-center gap-3">
            ...
            <h2 className="text-3xl font-black text-brand-navy dark:text-brand-gold tracking-tight">Latest News & Updates</h2>
          </motion.div>
          ...
          {profileData.news.map((item, idx) => (
            ...
            >
              <div className="flex justify-between items-start mb-6">
                <span className="px-3 py-1 bg-brand-gold/10 text-[9px] font-black text-brand-gold tracking-wide rounded-lg">{item.source}</span>
                <span className="text-[10px] font-bold text-slate-400 tracking-wide">{item.date}</span>
              </div>
              ...
              <div className="flex items-center gap-2 text-brand-gold text-[10px] font-black tracking-wide opacity-0 group-hover:opacity-100 transition-all">
